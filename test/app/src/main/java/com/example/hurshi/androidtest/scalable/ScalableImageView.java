...
public class ScalableImageView extends View implements GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener, Runnable {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        smallScale = Math.min((float) getWidth() / bt.getWidth(), (float) getHeight() / bt.getHeight());
...
//        pivotY = getHeight() / 2.0f;
    ...
    public boolean onDoubleTap(MotionEvent e) {
        pivotX = e.getX() - getWidth() / 2.0f;
        pivotY = e.getY() - getWidth() / 2.0f;

//        offsetX -= (pivotX - getWidth() / 2.0f) * (-1 + bigScale / smallScale);
//        offsetY -= (pivotY - getHeight() / 2.0f) * (-1 + bigScale / smallScale);
        if (scale >= bigScale) {
//            offsetX += (pivotX - getWidth() / 2.0f) * (-1 + bigScale / smallScale);
//            offsetY += (pivotY - getHeight() / 2.0f) * (-1 + bigScale / smallScale);
...
//            offsetY -= (pivotY - getHeight() / 2.0f) * (-1 + bigScale / smallScale);
    ...
    public void setScale(float scale) {
        pivotX += pivotX * (scale / this.scale - 1);
        pivotY += pivotY * (scale / this.scale - 1);

        offsetX -= pivotX;//* (scale > this.scale ? 1 : -1);
        offsetY -= pivotY;// * (scale > this.scale ? 1 : -1);
        this.scale = scale;
